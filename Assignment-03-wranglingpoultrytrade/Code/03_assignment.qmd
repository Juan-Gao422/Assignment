...
The minimum trade value for imports was \$4,308 from Poland in 2022.
The maximum trade value for imports was \$428,214,676 from Chile in 2022.
...
Chile had the highest total export value (\$1,532,686,723) to US.
...
The highest total import value (\$566,412,591) occurred in 2022.
...
```
The minimum trade value for exports was \$2,900 to Czechia in 2018.
The maximum trade value for exports was \$1,172,550,687 to Mexico in 2024.
...
In 2018, the top five countries to which the USA exported the most are Mexico (\$763,541,266), China, Hong Kong SAR (\$412,669,749), Angola (\$183,671,360), Other Asia, nes	(\$188,024,792), and Cuba (\$154,537,803).

In 2024, the top five countries to which the USA exported the most are Mexico (\$1,172,550,687), Cuba (\$298,231,173), Other Asia, nes	(\$261,812,562), Philippines (\$187,724,475), and Canada (\$403,487,221).
...
Mexico had the highest total import value (\$6,823,047,519) from US.
...
The highest total export value (\$5,401,343,250) occurred in 2022.
...
TotalValue <- PoultryDataClean %>%
  group_by(flow_desc) %>%
  summarise(sum(primary_value))

TotalValue

```

The total values of U.S. exports and imports from 2018 to 2024 were \$31042194412 and \$2427597118, respectively. Based on these data, the United States was a net exporter of poultry products during this period.

-   Based on number of countries US imports and exports

```{r}

TotalCountryNumber <- PoultryDataClean %>%
  group_by(flow_desc) %>%
  summarise(countrynumber = n_distinct(partner_desc))

TotalCountryNumber

```

The number of countries of U.S. exports and imports from 2018 to 2024 were 188 and 7, respectively. Based on these data, the United States was a net exporter of poultry products during this period.

-   Based on each year's import export value.

```{r}

Annualvalues <- PoultryDataClean %>%
  group_by(flow_desc,
            ref_year) %>%
  summarise(primary_value_annually = sum(primary_value)) %>%
  pivot_wider(names_from = flow_desc,
              values_from = primary_value_annually
              )
...
From 2018 to 2024, the U.S. export values were consistently greater than import values, indicating that the U.S. was a net exporter of poultry products during this period.
...
Totalvalues_Country <- PoultryDataClean %>%
  group_by(flow_desc,
           partner_desc) %>%
  summarise(primary_value_country = sum(primary_value)) %>%
  pivot_wider(names_from = flow_desc,
              values_from = primary_value_country) %>%
  arrange(desc(Import))
...
The main import partners are not necessarily the major export destinations. For example, Chile and Canada play key roles in both imports and exports. However, several key import sources, such as Israel and Germany, are not major export markets. Conversely, major export destinations like Mexico and China account for large export values but very small import values. 
...
TradeSituation <- PoultryDataClean %>%
  group_by(flow_desc,
            ref_year) %>%
  summarise(primary_value_annually = sum(primary_value)) %>%
  pivot_wider(names_from = flow_desc,
  ...
  mutate(Tradesituation = Import - Export)
...
Based on the calculation (Imports − Exports), all values are negative from 2018 to 2024, indicating that the U.S. experienced a trade surplus in poultry products in every year during this period.
...
- Some potential issues you may encounter and how to fix them:   
  - Some times your code may be creating a table output that is waaay too long and cumbersome to scroll through when rendered. If this is the case, make it more professional looking by using the `head()` function to only print the first handful of rows (instead of thousands of rows).  
